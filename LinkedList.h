...
class LinkedList
{
    ...
    bool deletePosition(int pos); /* - method to remove the node at position pos from the list and delete it. 
                        If it successfully deletes the node, it should return true. If pos is out-of-bounds, it should return false.*/
    ...
    int search(int target) ;/*which searches the list for the first occurrence of target in the list and returns the index of where target is. 
    If target does not exist in the list, return -1.*/
    
    void printList() ;/*- method that prints the data of all the nodes in the list, separated by spaces and encapsulated by '[' and ']'. 
    In the case of an empty list, it prints nothing.*/
};
